...
1. Put the verified official format document under `00_rules/official/`. For the currently verified 2026 binary, compare the hash with `cumcm-2026-format-fingerprint.md`; if it differs, inspect the new source before replacing the lock.
...
- Electronic paper is one uncompressed standalone PDF or Word file no larger than 20 MB; supporting archive is one ZIP/RAR no larger than 20 MB.
- Electronic and printed paper content and formatting, including appendices, are identical.
- The appendix is part of the printed paper, lists every supporting file, and includes all complete runnable source code or the required no-program statement.
