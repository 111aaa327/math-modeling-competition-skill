...
- Organize the paper around the requested questions and decisions, not a catalog of algorithms. Use headings that identify the actual mechanism or task; avoid a repeated fill-in-the-blank ladder such as `问题一的模型建立与求解` when a more informative heading is available.
- Make the abstract state, for every requested subproblem, the main method, quantitative finding, validation, and practical meaning within the official limit. Keep it consistent with the body and result files; avoid complex formulas, tables, and citations.
- Define symbols before use and keep notation, units, precision, and terminology consistent.
- State assumptions with necessity, rationale, affected equations/results, consequences, and validation, not arbitrary quantity targets.
- Every table and figure needs a purpose, readable labels/units, a data source, a preceding text reference, and a following interpretation. Figure captions belong below figures and table captions above tables unless the locked template or official rule says otherwise.
...
- Treat the official page and file format as a presentation lock, not a universal content template. The argument, assumptions, model, validation, and headings must remain problem-specific.
...
```text
python scripts/paper_preflight.py --paper <paper.docx|main.tex|main.typ|paper.md> --project <workspace> --registry <optional-result-registry.json> --support-archive <optional-support.zip> --output <report.json>
